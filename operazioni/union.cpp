...
//funtore di filter per la distinct
...
public:
    bool operator()(SensorInput &input) {
        ...
        //second è true solo se non c'era già nella hashmap
...
public:
    void operator()(std::optional<SensorInput> &input) {
        ...
        std::cout << "[Union] Sensore: " << input->sensor_id 
...
int main() {
    ...
    auto distinct_op = wf::Filter_Builder(dist_func)
        .withParallelism(3)
    ...
    wf::PipeGraph topology("TableAPI_UnionQuery", 
    ...
    merged_pipe->add(distinct_op).add_sink(sink_op);
