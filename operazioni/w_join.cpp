...
//Svolge:
//  w = Windows.createTBWindow(2000000)    //tumble di due secondi
//  q = tab1.join(tab2, "sensor_id", w).select("sensor_id", "temperature", "temp")
...
int main() {
    ...
    Source_Functor src2_func("sensor_stream_input2.csv");
    ...
    auto window_join = wf::Window_Join_Builder(join_func)
        .withName("EquiJoin_Sensors")
        .withParallelism(3)
        .withKeyBy([](const SensorInput &in) -> std::string { 
            return in.sensor_id; 
        })
        .withKPMode() 
        .withTumblingWindows(std::chrono::microseconds(2000000)) 
        .build();
    
    auto sink = wf::Sink_Builder(snk_func).withName("Sink").build();

    wf::PipeGraph topology("TableAPI_SelfJoinQuery", 
    ...
    merged_pipe->add(window_join).add_sink(sink);
