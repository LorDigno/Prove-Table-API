...
struct SelectedSensor {
    ...
    bool operator==(const SelectedSensor& other) const {
        return (sensor_id == other.sensor_id) && (humidity == other.humidity);
    }
};

//aggiungo l'hash per SelectedSensor in std
namespace std {
    template <>
    struct hash<SelectedSensor> {
        std::size_t operator()(const SelectedSensor& k) const {
...
struct SelectedKey {
    ...
    bool operator==(const SelectedKey& other) const {
        return (humidity == other.humidity);
    }
};

//aggiungo l'hash per SelectedKey in std
namespace std {
    template <>
    struct hash<SelectedKey> {
        std::size_t operator()(const SelectedKey& k) const {
            return std::hash<double>{}(k.humidity);
        }
    };
}
...
int main() {
    ...
    auto distinct_key = [](const SelectedSensor& in) -> SelectedKey {
        return SelectedKey{in.humidity};
    }; 

    //distinct keyed 
    auto distinct_op = Distinct_Builder<SelectedSensor, SelectedKey>()
        .withName("distinct")
    ...
    auto distinct_op = Distinct_Builder<SelectedSensor>()
        .withName("distinct")
        .withParallelism(1)     //è senza chiave quindi per forza
        .build();
    */
